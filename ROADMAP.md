...
## Phase 4.5: ピクセル単位アノテーション(SAM によるマスク生成)

- [x] SAM2 / SAM3 の事前検証 (2026-07-14, COCO person 203インスタンス, GT bbox プロンプト):

  | モデル | 平均IoU | 中央値 | 小(面積<2%) | 大 | IoU<0.7 |
  |--------|---------|--------|--------------|-----|---------|
  | SAM2 base (162MB) | 0.786 | 0.818 | 0.732 | 0.863 | 23.6% |
  | SAM3 (3.45GB, gated) | 0.794 | 0.822 | 0.753 | 0.852 | 23.6% |

  結論: bbox プロンプトでは両者同等 → 量産は SAM2 を採用(SAM3 は20倍重く M2 では数倍遅い)。
  CrowdHuman 密集シーン(最大377人/枚)でも目視で破綻なし(data/viz/sam/)。
  MPS では箱プロンプトを24個ずつに分割しないと OOM になる。
  SAM3 のテキストプロンプト機能は将来 GT なし画像の検出改善用に再検討の価値あり(クラウド前提)。
- [x] モデル選定 (2026-07-14, ユーザー目視判断): **SAM3 + GT bbox プロンプト**を採用。
  - 数値ベンチマーク (COCO GT との IoU) は正解自体が粗いため参考値に留める方針
  - SAM3 テキストプロンプト ("person") は背景の観客まで拾える (16→59人/枚, imgsz=1008) が、
    主要人物のマスクが bbox プロンプトより崩れるため不採用
  - **背景の人物 (GT にない) は人手で追加アノテーションする**
- [ ] sam_annotate.py で SAM3 少量バッチ生成 (--max-images) → FiftyOne で確認 → 人手修正の反復
  - 運用値 (8GB Mac): SAM3 はプロセスが 8GB 超まで肥大しスワップで停止するため
    **5枚ごとにプロセス再起動** (`--max-images 5` を外側ループで回す)。
    Docker Desktop 等の常駐メモリ食いは必ず終了させてから実行する
- [ ] 背景人物の人手アノテーション手段の整備 (CVAT 連携 or FiftyOne プラグイン)
